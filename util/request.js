...
const createRequest = (uri, data, options, retryCount = 0) => {
  return new Promise((resolve, reject) => {
    ...
    // 设置超时
    settings.timeout = options.timeout || REQUEST_TIMEOUT
    ...
    axios(settings)
      .then((res) => {
        ...
        if (answer.status === 200) {
          ...
          // [FIX-B001] 2026-07-28 | 后端 | 安全
          // 问题：429/403 响应不进重试逻辑，直接 reject 导致上层崩溃
          // 修改：429/403/502 标记为可重试错误，在 catch 中统一处理
          // 状态：已修复
          if ([429, 403, 502].includes(answer.status)) {
            answer._retryable = true
          }
      ...
      .catch((err) => {
        ...
        const isRetryable = isNetworkError || (err._retryable === true)

        if (isRetryable && retryCount < MAX_RETRIES) {
          const delay = RETRY_BASE_DELAY * Math.pow(2, retryCount)
          console.log(`[RETRY] ${uri} 第 ${retryCount + 1} 次重试，${delay}ms 后重试`, `(${err.code || err.status || err.message})`)
          setTimeout(() => {
            createRequest(uri, data, options, retryCount + 1)
              .then(resolve)
              .catch(reject)
          }, delay)
          return
