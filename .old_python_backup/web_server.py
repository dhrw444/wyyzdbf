...
task_manager = {
    ...
    'thread_pool': None,
    'max_threads': 4,
    'login_sessions': {}  # phone -> requests.Session, 保持短信发送与登录同 session
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_start_task(self):
        ...
        command = self.get_task_command(task_type)
    ...
    def handle_send_code(self):
        ...
        # 清理该手机号旧的残留 session
        old = task_manager['login_sessions'].pop(phone, None)
        if old:
            old.close()
        ...
        try:
            ...
            import uuid
            # 创建共享 Session，补齐全套指纹 Cookie + 页面预热
            ...
            session.cookies.set("NMTID", uuid.uuid4().hex, domain="music.163.com")
            session.cookies.set("_ntes_nnid", uuid.uuid4().hex, domain="music.163.com")
            session.cookies.set("_ntes_nuid", uuid.uuid4().hex, domain="music.163.com")
            # 关键：先访问首页预热会话，模拟浏览器打开页面（此时不设 Referer）
            try:
                session.get("https://music.163.com/", timeout=10)
            except Exception:
                pass
            # 预热后补充 API 请求用的 Referer + Origin
            session.headers.update({
                "Referer": "https://music.163.com/",
                "Origin": "https://music.163.com"
            })
            print(f"[DEBUG] 开始发送验证码，手机号: {phone}, csrf: {csrf[:8]}...")
            ok, detail = send_sms_code(session, phone)
            print(f"[DEBUG] send_sms_code 返回: ok={ok}, detail={detail}")
            if ok:
                task_manager['login_sessions'][phone] = session
                self._send_json({"success": True, "message": "短信验证码已发送", "expires": 120})
            else:
                self._send_json({"success": False, "message": detail}, 400)
        except Exception as e:
            print(f"[ERROR] 发送验证码异常: {e}")
            import traceback
            traceback.print_exc()
            self._send_json({"success": False, "message": "发送失败: " + str(e)}, 400)
    ...
    def handle_verify_login(self):
        ...
        if not re.match(r'^\d{6}$', code):
            self._send_json({"success": False, "message": "验证码必须6位数字"}, 400)
            return

        session = task_manager['login_sessions'].get(phone)
        if session is None:
            self._send_json({"success": False, "message": "请先发送验证码"}, 400)
            return

        try:
            from netease_player import login_with_captcha
            ok, detail = login_with_captcha(session, phone, code)
            if ok:
                del task_manager['login_sessions'][phone]
                session.close()
                new_id = max(task_manager['accounts'].keys(), default=0) + 1
                account = Account(
                    id=new_id,
            ...
            self._send_json({"success": False, "message": "验证失败: " + str(e)}, 400)
    ...
    def get_task_command(self, task_type='play'):
        """根据任务类型获取任务命令"""
        commands = {
            'play': 'search 周杰伦 -p',
            'stats': 'stats',
        ...
        return commands.get(task_type, 'search 周杰伦')
