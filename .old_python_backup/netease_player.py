...
def _api_post(url, data, cookies=None, session=None, custom_headers=None):
    headers = {
    ...
    print(f"[DEBUG] _api_post 发送请求:")
    print(f"  URL: {url}")
    print(f"  原始数据: {data}")
    print(f"  加密后: params长度={len(encrypted['params'])}, encSecKey长度={len(encrypted['encSecKey'])}")
    try:
        ...
        # HTTP 错误时也尝试解析响应体拿到网易云返回的实际错误
        try:
            body = e.response.json()
            print(f"  HTTP {e.response.status_code} 响应: {body}")
            return body
        except Exception:
            pass
        print(f"  请求异常: {e}")
...
def send_sms_code(session, phone, countrycode="86"):
    """发送短信验证码，返回 (success, detail_message)"""
    ...
    print(f"send_sms_code 响应: code={code}, msg={msg}")
    if code == 200:
        print("短信验证码已发送")
        return (True, "验证码已发送")
    return (False, msg or "发送失败")

def login_with_captcha(session, phone, captcha, countrycode="86"):
    """验证码登录，返回 (success, detail_message)"""
    csrf = session.cookies.get("__csrf", "")
    data = {
        "phone": phone,
        "captcha": captcha,
    ...
    result = _api_post("https://music.163.com/weapi/sms/captcha/verify", data, session=session)
    if result.get("code") == 200:
        ...
        return (True, "登录成功")
    msg = result.get("message", "")
    print(f"登录失败: code={result.get('code')}, msg={msg}")
...
def login_cellphone(phone, password=None, countrycode="86", skip_captcha=False):
    ...
    result = _api_post(
        ...
        {"phone": phone, "password": "", "countrycode": countrycode},
        session=session,
    )
    code = result.get("code")
    if code != 200 and code not in (400, 501, 502):
        print(f"发送短信失败: code={code}")
        return False
